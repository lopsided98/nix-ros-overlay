...
}:
{
  ...
  tarSource =
    ...
    src:
    self.runCommand ("${src.name}.tar" + lib.optionalString compress ".gz")
      (
        {
          inherit src;
        }
        ...
        postPatch =
          ''
            substituteInPlace ${lib.escapeShellArg file} \
              --replace-fail ${lib.escapeShellArg originalUrl} ${
                lib.escapeShellArg (self.fetchurl { inherit url hash sha256; })
              }
          ''
          + postPatch;
      }
  ...
  patchGzAmentVendorGit =
    ...
    let
      stem = lib.strings.removeSuffix "-vendor" (
        lib.strings.removePrefix "ros-${pkg.rosDistro}-" pkg.pname
      ); # e.g. gz-cmake
